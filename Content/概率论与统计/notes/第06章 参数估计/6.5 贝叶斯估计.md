...
>
> 即 $\sigma^2|\boldsymbol{x} \sim IGa\!\left(\alpha+\frac{n}{2},\; \lambda+\frac{1}{2}\sum_{i=1}^{n}(x_i-\mu_0)^2\right)$，仍为倒伽马分布，得证。$\blacksquare$
...
>
> 这是参数为 $n+\beta+1$ 与 $\max\{x_{(n)},\theta_0\}$ 的帕雷托分布，得证。$\blacksquare$
>
> **(2)** 后验期望估计：
>
...
>
> 归一化后即为狄利克雷分布 $D(\boldsymbol{\alpha}+\boldsymbol{x})$，其中 $\boldsymbol{\alpha}+\boldsymbol{x} = (\alpha_1+x_1, \alpha_2+x_2, \cdots, \alpha_k+x_k)$。$\blacksquare$
