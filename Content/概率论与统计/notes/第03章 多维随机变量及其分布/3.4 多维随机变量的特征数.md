...
>
> 因此 $\text{Var}(X + Y) = \text{Var}(X) + \text{Var}(Y)$。$\blacksquare$
...
>
> $\text{Var}(X - Y)$ 类似，交叉项取负号。$\blacksquare$
...
> $= \text{Cov}(X_1, Y) + \text{Cov}(X_2, Y)$。$\blacksquare$
...
>
> 即 $[\text{Cov}(X, Y)]^2 \leq \text{Var}(X)\,\text{Var}(Y)$。$\blacksquare$
...
> [!abstract]
> **证明思路**：$|\rho| = 1$ 等价于柯西-施瓦茨不等式取等号，等价于二次函数 $g(t)$ 有重根 $t = t_0$，即 $g(t_0) = 0$。而 $g(t_0) = E[(X - EX + t_0(Y - EY))^2] = 0$ 意味着 $X - EX + t_0(Y - EY) = 0$ 几乎必然成立，即 $Y = aX + b$ 几乎必然成立。$\blacksquare$
