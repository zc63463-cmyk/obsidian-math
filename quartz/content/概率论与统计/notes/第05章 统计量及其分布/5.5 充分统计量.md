...
> >
> > 因此，$T = \sum_{i=1}^{n} X_i$ 是 $\theta$ 的充分统计量。 $\square$
...
> **第三步：得因子分解**
>
> 则 $p(x_1, \ldots, x_n;\, \theta) = g(T(x_1, \ldots, x_n),\, \theta) \cdot h(x_1, \ldots, x_n)$，因子分解成立。 $\square$
...
> **第三步：与 $\theta$ 无关**
>
> 条件分布 $p(x_1, \ldots, x_n \mid T = t) = \dfrac{h(x_1, \ldots, x_n)}{H(t)}$ 中不含 $\theta$，因此 $T$ 是 $\theta$ 的充分统计量。 $\square$
...
> $$
> $g^*$ 仅通过 $S$ 依赖于样本，$h$ 不含 $\theta$，因此 $S$ 也是 $\theta$ 的充分统计量。 $\square$
...
> $$
>
> $g$ 仅通过频数向量 $T'$ 依赖于样本，$h$ 不含 $\theta$。因此频数向量也是充分统计量。 $\square$
...
> $$
>
> $g$ 仅通过 $T = (S_1, \ldots, S_k)$ 依赖于样本，$h^*$ 不含 $\theta$。因此 $T$ 是 $\theta$ 的充分统计量。 $\square$
